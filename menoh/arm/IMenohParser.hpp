...
namespace menoh_impl {
    namespace armnn_backend {
        ...
        class IMenohParser;
        using IMenohParserPtr = std::unique_ptr<IMenohParser, void(*)(IMenohParser* parser)>;

        /// parses a directed acyclic graph from a tensorflow protobuf file
    class IMenohParser
    {
      public:
      static IMenohParser* CreateRaw();
      static IMenohParserPtr Create();
      static void Destroy(IMenohParser* parser);

    /// Create the network from a protobuf text file on disk
    virtual armnn::INetworkPtr CreateNetworkFromTexMenohile(
